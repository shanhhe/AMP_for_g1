...
import rsl_rl
from rsl_rl.algorithms import AMPPPO, PPO, Distillation
...
class G1AMPOnPolicyRunner:
    ...
    def __init__(self,
        ...
        if self.runner_cfg["algorithm_class_name"] == "AMPPPO":
            self.training_type = "rl"
        elif self.runner_cfg["algorithm_class_name"] == "Distillation":
        ...
        alg_class = eval(self.runner_cfg.pop("algorithm_class_name"))  # PPO
        min_std = (
            torch.tensor(self.runner_cfg["min_normalized_std"], device=self.device) *
            (torch.abs(self.env.dof_pos_limits[:, 1] - self.env.dof_pos_limits[:, 0])))
        self.alg: AMPPPO | Distillation = alg_class(
            policy, discriminator, amp_data, amp_normalizer, device=self.device, min_std=min_std, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg
            )
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                ...
                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.runner_cfg)
                self.writer.log_config(self.env.cfg, self.runner_cfg, self.alg_cfg, self.policy_cfg)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # Move to device
                    obs, next_amp_obs, rewards, dones = (obs.to(self.device), next_amp_obs.to(self.device), rewards.to(self.device), dones.to(self.device))
                ...
                # compute returns Generalized Advantage Estimation（GAE)
                if self.training_type == "rl":
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if locs["it"] % 10 == 0:
            ...
            if len(locs['rewbuffer']) > 0:
                ...
                log_string += f"""{'Mean AMP rewards:':>{pad}} {statistics.mean(locs["amprewardbuffer"]):.2f}\n"""
                log_string += f"""{'Mean task rewards:':>{pad}} {statistics.mean(locs["taskrewardnbuffer"]):.2f}\n"""
